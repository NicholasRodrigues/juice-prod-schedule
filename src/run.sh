...
for instance in "$DATA_DIR"/*.txt; do
    ...
    # Run the program 5 times for each instance
    for i in {1..5}; do
        ...
        seed=$(echo "$output" | grep "Using Seed" | awk '{print $NF}')
        ...
        echo "Run #$i - Seed: $seed, Penalty: $penalty, GAP: $gap%, Execution Time: $execution_time seconds" >> "$result_file"
    ...
    echo "Best Seed: $best_seed" >> "$result_file"
    echo "Schedule: $best_schedule" >> "$result_file"
    echo "--------------------------------" >> "$result_file"

    # Append the best result to a summary file
    echo "$instance_name - Best Penalty: $best_penalty, Best GAP: $best_gap%, Best Seed: $best_seed, Execution Time: $best_execution_time seconds" >> "$RESULTS_DIR/best_results_summary.txt"
done
